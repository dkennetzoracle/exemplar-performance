...
Both recipes are validated for their listed NVIDIA Grace SKUs. See the per-recipe
...
4. **`tmux` or `screen` strongly recommended** for installs. The model fetch step
   ...
   export LLMB_INSTALL=/path/to/llmb-install
...
Concrete example, with `LLMB_INSTALL=/path/to/llmb-install` and the GLM5
recipe:

```text
/path/to/llmb-install/
└── workloads/
    └── inference_glm5/
        └── srt-slurm/
            └── outputs/
                └── <benchmark-jobid>/                         # benchmark SLURM job id
                    ├── config.yaml                  # recipe used for this run
                    └── logs/
                        ├── sweep_<benchmark-jobid>.log        # orchestrator log (worker startup, health checks)
...
Requires cluster support for `--segment` (GB200 NVL72 clusters may support it). If your cluster rejects it with `sbatch: error: Invalid --segment specification`, set the flag back to `false` and contact your cluster admin about NVLink-aware allocation. See [Internals: SBATCH directive compatibility](#sbatch-directive-compatibility) for the full flag table.
