...
Meanwhile switching bf16 -> fp8 at MBS=1 made it **37% slower**. So for this
MoE, **MBS dominates precision by a wide margin**. VR200 cannot run fp8 at
all, but it can raise MBS — a portable win costing nothing but a config
change. If 3.10x reproduces on sm_107, VR200's Qwen3 ceiling moves from
9,317 to roughly 29,000 tok/s/GPU, which would put it *ahead* of GB300's
best MoE number. Worth knowing before concluding a bf16-only machine is
out of headroom.
