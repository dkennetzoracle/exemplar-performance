...
| `SUMMARY.md` | **start here** — one-page version: headline, raw tables, and what each lever (MBS/GBS/recompute/precision) was worth per workload |
...
| `comparison.csv` | the same table as CSV, one row per config, plus a `config` shorthand column and an explicit `*_status` column per machine. Generated by `make_plots.py` |
| `plots/` | one grouped-bar chart per workload, `tok/s/GPU`, panelled by container. Generated by `make_plots.py` |
